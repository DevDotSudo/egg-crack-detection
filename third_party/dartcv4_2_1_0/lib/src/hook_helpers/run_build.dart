...
Future<void> runBuild(BuildInput input, BuildOutputBuilder output, {Set<String>? optionalModules}) async {
  ...
  // dartcv4 2.1.0 downloads an OpenCV Windows pack that contains x64/vc17
  // binaries. Visual Studio 18 Preview reports a newer MSVC_VERSION than the
  // generated OpenCVConfig.cmake recognizes, leaving OpenCV_RUNTIME empty.
  // Passing the runtime explicitly lets the package use its bundled vc17 libs.
  final windowsOpenCvDefines = input.config.code.targetOS == OS.windows
      ? const {
          'OpenCV_ARCH': 'x64',
          'OpenCV_RUNTIME': 'vc17',
        }
