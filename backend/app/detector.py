...
def _inner_egg_mask(egg_mask: np.ndarray, cfg: DetectionConfig) -> np.ndarray:
    ...
    egg_points = cv2.findNonZero(egg_mask)
    if egg_points is not None:
        _, egg_y, _, egg_h = cv2.boundingRect(egg_points)
        support_cutoff = egg_y + int(round(egg_h * 0.83))
        inner[support_cutoff:, :] = 0
...
def _paper_method_crack_detection(
    ...
    distance = cv2.distanceTransform(binary_egg, cv2.DIST_L2, 5)
    max_distance = float(distance.max())
    safe_margin = max(14.0, max_distance * 0.12)
    safe_inner = np.where(distance > safe_margin, 255, 0).astype(np.uint8)
    egg_points = cv2.findNonZero(binary_egg)
    if egg_points is not None:
        _, egg_y, _, egg_h = cv2.boundingRect(egg_points)
        support_cutoff = egg_y + int(round(egg_h * 0.82))
        safe_inner[support_cutoff:, :] = 0
...
def _capture_quality_metrics(
    ...
    blurry = bool(
        ...
        or (
            laplacian_variance < cfg.quality_min_laplacian_variance
...
def _prepare_candling_image(
    ...
    detail_dark_preserved = cv2.medianBlur(clahe.apply(blended), 3)
...
def _is_thin_texture_crack(
    ...
    metrics = component.metrics
    return bool(
        metrics['skeleton_length'] >= cfg.thin_crack_min_length
...
def _component_score(metrics: dict[str, float], cfg: DetectionConfig) -> float:
    score = 0.0
    score += min(metrics['skeleton_length'] / 70.0, 1.8)
    score += min(metrics['span'] / 65.0, 1.4)
    ...
    score -= max(0.0, metrics['average_thickness'] - cfg.preferred_max_thickness) * 0.16
    score -= max(0.0, metrics['density'] - 0.45) * 2.6
    score -= metrics['rim_overlap'] * 1.5
    return score


def _has_strict_line_geometry(metrics: dict[str, float]) -> bool:
    """Require an actual thin trace, not merely a locally dark shell region."""
    return bool(
        metrics['average_thickness'] <= 4.0
        and metrics['max_thickness'] <= 7.0
...
def _is_crack_seed(
    ...
    if (
        ...
        return False
    if source == 'paper' and not _has_strict_line_geometry(metrics):
        return False
...
def detect_image_bytes(
    ...
    *,
    ...
    persistent_bright_components = _mask_to_components(
    ...
    for component in persistent_dark_components + persistent_bright_components:
        _paste_component(trusted_local_mask, component)
    ...
    decision_score = 0.0
    if components:
        decision_score += min(longest / 85.0, 1.8)
        decision_score += min(total_length / 150.0, 1.2)
        decision_score += min(strongest / 55.0, 0.8)
        decision_score += min(best_score / 4.0, 1.0)
        decision_score += min(traced_pixels / 160.0, 0.6)
    ...
    thin_crack_score = float(max((
        min(component.metrics['skeleton_length'] / 45.0, 1.0) * 0.28
        + min(component.metrics['span'] / 45.0, 1.0) * 0.20
...
def detect_camera_image_bytes(
    ...
    if camera_cfg.camera_fast_mode:
        result = detect_image_bytes(data, include_steps, camera_cfg)
...
def detect_camera_images_bytes(
    ...
    for frame in frames[:cfg.multi_frame_count]:
        try:
            frame_results.append(detect_camera_image_bytes(frame, include_steps, cfg))
    ...
    candidates = [
        (index, result, _decode_crack_mask(result))
    ...
    if len(stable_results) < cfg.multi_frame_min_agreement:
        ...
        return _clear_unconfirmed_crack(
            representative,
    ...
    result.update({
        'processing_time_ms': sum(
            int(frame['processing_time_ms']) for frame in frame_results
        ),
        'sample_count': sample_count,
